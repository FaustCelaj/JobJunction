...
import { Box, Typography, Button, Divider, Card, CardContent, CardActions, Grid, Snackbar } from '@mui/material';
  ...
  return (
    <Card sx={{ maxWidth: 600, margin: 'auto', mt: 5, p: 2 }}>
      <CardContent>
        ...
        <Typography variant="body1">Industry: {job.company.industry}</Typography>
        <Typography variant="body1">Size: {job.company.companySize}</Typography>
        <Typography variant="body1">Location: {job.company.location}</Typography>
        <Typography variant="body1">Website: <a href={job.company.website} target="_blank" rel="noopener noreferrer">{job.company.website}</a></Typography>
      </CardContent>
      <CardActions>
